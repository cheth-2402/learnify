...
proxy_host = "http://10.10.78.22"
proxy_port = 3128
proxy_user = "cs1210561"
proxy_pass = "aikumask"

mail = Mail(app)
mail.init_app(app)
app.secret_key = '^$*Hdd68*f'
app.config["F"]="/home/baadalvm/Learnify/project/static/imgs"
...
def getVideoAndReview(course_id):
    if 'username' in session:
        with connection.cursor() as cursor:
            ...
            user = cursor.fetchone()
            connection.commit()
        return render_template("course.html", videos = videos,reviews = reviews,course=course,user=user,u=session["id"],vstring="tab-pane fade show active",rstring="tab-pane fade",vnstring="nav-link active",rnstring="nav-link")
    else:
        flash("login required")
        return redirect("/login")
...
def translater(courseid):
    if "username" in session:
        ...
        return render_template("course.html", videos = videos,reviews = reviews,course=course,user=user,u=session["id"],vstring="tab-pane fade",rstring="tab-pane fade show active",vnstring="nav-link",rnstring="nav-link active")
    else:
        flash("login required")
        return redirect("/login")
